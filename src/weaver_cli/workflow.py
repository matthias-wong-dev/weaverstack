...
def load_workflow(name: str, *, file: str | None = None) -> tuple[list[str], Path]:
    ...
    if not path.is_file():
        action = "Check --file." if file else "Create workflow.yml or pass --file."
        raise CommandError(f"Workflow file not found: {path}. {action}")
    try:
    ...
    if not isinstance(document, dict):
        raise CommandError(f"{path}: expected a top-level mapping")
    workflows = document.get(WORKFLOW_KEY)
    if workflows is None:
        raise CommandError(f"{path}: missing {WORKFLOW_KEY!r} key")
    if not isinstance(workflows, dict):
        raise CommandError(f"{path}: {WORKFLOW_KEY!r} must be a mapping of names")
    if name not in workflows:
        known = ", ".join(sorted(workflows)) or "none"
        raise CommandError(f"{path}: workflow {name!r} not found. Available: {known}")
...
def run_workflow(args: argparse.Namespace, *, parser_factory=None, stdin=None) -> int:
    ...
    # Parse every entry and resolve the Workspace before asking for authorisation.
    parsed_commands = [_parse(parser, entry) for entry in entries]
    # Interaction policy propagates inward and can only become stricter.
    ...
    if not authorised(args):
        ...
        if not can_prompt(args, stream):
            print(
                "Pass --yes to run this workflow without a prompt.",
                file=sys.stderr,
            )
            return 1
...
def _workflow_workspace(args, parsed_commands):
    """Resolve the single Workspace shared by the workflow commands."""
    ...
    if len(workspaces) > 1:
        raise CommandError(
            "Workflow commands name different workspace configurations; "
            "a workflow runs in one Workspace."
        )
    return workspaces[0]


def _warm_for(session, parsed_commands, *, workspace) -> None:
    """Start shared resources before the first command.

    Offer all Lakehouses before preparation so Spark can attach once for the sequence.
    """
