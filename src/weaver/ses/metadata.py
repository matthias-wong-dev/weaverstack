...
#: The identity column is a Weaver-managed surrogate: build creates it as a plain
#: not-null ``bigint`` — never engine-autogenerated — and a later load populates
#: it. It is Weaver's column, so it is not part of the declared business schema
#: or a query's output. ``bigint`` is valid in both Spark SQL and T-SQL.
...
class SesDocument:
    ...
    def identity_column(self) -> Column | None:
        """The Weaver-managed surrogate column, when the Identity header names one.

        A plain not-null ``bigint`` that build creates and a later load populates
        — never engine-autogenerated. It is Weaver's own column, so it stands
        outside the business schema (declared or inferred); the primary key may
        name it when the surrogate is the key.
        """
...
def _validate_columns(
    *,
    ...
    if identity is not None and identity in audit_names:
        raise MetadataError(
