...
to the run, the same row the Warehouse `_.Load` procedure writes, and health
reads it as no change: the skip is passed over in the ancestry walk and its
timestamp is not data movement.
