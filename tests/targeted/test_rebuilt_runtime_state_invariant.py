...
@weaver_test()
def test_every_rebuilt_loadable_is_established_as_unloaded(rebuilt, tmp_path):
    ...
    loadable = _bookmarkable(_estate(tmp_path / "declared"))

    rebuilt_loadables = {
        identity for identity in _physically_rebuilt(rebuilt) if identity in loadable
    }
    assert rebuilt_loadables, "the estate rebuilt no loadable, so this proves nothing"

    unreset = {
        identity
        for identity in rebuilt_loadables
        if statuses.get(identity.rsplit(".", 1)[-1], {}).get("result") != PENDING
    }
    assert unreset == set()
    ...
    without_sentinel = {
        identity
        for identity in rebuilt_loadables
        if bookmarks.get(identity.rsplit(".", 1)[-1], {}).get("bookmark_datetime")
        != BOOKMARK_SENTINEL_TEXT
    }
