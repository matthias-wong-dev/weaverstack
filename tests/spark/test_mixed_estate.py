"""The Lakehouse side of a mixed estate, on local Spark.
...
is installed once per module (``local_lakehouse_estate``) and every assertion reuses it.

**Local only.** Every claim here is about what the planner decided and what DDL
came out of it: which nodes were omitted, what columns a built table has, whether
a view resolves through its chain. None of that differs by transport, and the
Fabric half used to run this same body against these same assertions for a whole
extra install. The Fabric-specific claims about this material — a real catalogue
read, a SQL endpoint, an in-session build — live in the modules that make them.
"""
...
def test_the_warehouse_leaves_are_omitted_from_the_lakehouse_build(local_lakehouse_estate):
    ...
    omitted = {node.node_id for node in local_lakehouse_estate.bundle.plan.omitted_nodes}
    assert {
...
def test_the_folder_and_tables_are_built_empty_with_audit_columns(local_lakehouse_estate):
...
def test_the_spark_view_resolves_through_the_chain(local_lakehouse_estate):
    env = local_lakehouse_estate.env
    view_columns = {
        column["name"].lower()
