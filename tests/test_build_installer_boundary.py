...
@weaver_test()
def test_unsupported_identity_is_refused_before_the_existing_table_is_dropped(
    tmp_path,
):
    import hashlib
    import json
    ...
    drop_payload = b"DROP TABLE `Demo`.`Sales_LH`.`DWG`.`Customer`\n"
    table_payload = (
        json.dumps(
            {
                "object": "`Demo`.`Sales_LH`.`DWG`.`Customer`",
                "schema_mode": "declared",
                "declared_columns": [["CustomerId", "string", True]],
                "source_query": None,
                "setup": [],
    ...
    drop = InstallAction(
        id="drop-customer",
        kind="drop_table",
        resource_node_id="Lakehouse/Sales/Tables/DWG.Customer",
        executor="spark_sql",
        payload="payload/drop.spark.sql",
        payload_sha256=hashlib.sha256(drop_payload).hexdigest(),
    )
    create = InstallAction(
        ...
        payload="payload/create.spark-table.json",
    ...
    plan = BuildPlan(
        ...
        sequences=(
            BuildSequence(
                ...
                batches=(
                    BuildBatch(id="b", target_id=TARGET.id, actions=(drop, create)),
                ),
            ),
        ),
        selection=BuildSelection(Impact((), (), ()), (), (), ()),
    ...
    store = FilesystemStore()
    bundle = write_bundle(
        Location(str(tmp_path / "identity-bundle")),
        plan=plan,
        payloads={
            "payload/drop.spark.sql": drop_payload,
            "payload/create.spark-table.json": table_payload,
        },
        store=store,
    )

    class Unsupported(TestSession):
        def create_delta_table(self, qualified_name, columns, **kwargs):
            self.probes.append((qualified_name, columns, kwargs))
            ...
            return super().create_delta_table(qualified_name, columns, **kwargs)

    base = given_session(store=store)
    session = Unsupported(
    ...
    with pytest.raises(CommandError, match="Customer.*IdentityGenerator"):
        Installer(session).install(bundle)

    assert session.spark_sql == ()
    assert session.probes[0][0] == "`Demo`.`Sales_LH`.`DWG`.`Customer`"
