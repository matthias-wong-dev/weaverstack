...
- a load reads the foreign estate through a table, a folder and a schema
  shortcut;
- an unkeyed incremental Delta table appends immutable versions under a generated
  identity, and an unchanged source window is a no-op;
...
def _estate_evidence(journey) -> dict:
    ...
    return {
        ...
        "cur_customer_history": (
            "select ChangeKey, CustomerId, CustomerName, UpdatedAt "
            f"from {curated}.`CUR`.`CustomerHistory` order by ChangeKey"
        ),
...
def test_seeded_foreign_data_flows_through_every_layer(acceptance):
    ...
    history = seen["cur_customer_history"]
    assert _ids(seen, "cur_customer_history", "CustomerId") == [1, 2, 3]
    assert len({row["ChangeKey"] for row in history}) == 3
    assert all(int(row["ChangeKey"]) > 0 for row in history)
...
def test_an_unchanged_load_moves_only_the_appending_branch(acceptance):
    ...
    Proof: a second load with no source change leaves current and historical
    customer rows alone, adds exactly one generated event file, and leaves the
    stable branches equal.
    """
    ...
    assert after["cur_customer_history"] == before["cur_customer_history"]
...
def test_foreign_source_movement_propagates_through_the_whole_chain(acceptance):
    ...
    before_history = {row["ChangeKey"]: row for row in before["cur_customer_history"]}
    after_history = {row["ChangeKey"]: row for row in after["cur_customer_history"]}
    assert {key: after_history[key] for key in before_history} == before_history
    appended = [
        after_history[key] for key in after_history.keys() - before_history.keys()
    ]
    assert sorted((row["CustomerId"], row["CustomerName"]) for row in appended) == [
        (2, "Bob Updated"),
        (4, "Diana"),
    ]
    assert 3 in {row["CustomerId"] for row in after_history.values()}
...
#: statement runs.
#:
#: ``CUR.Customer`` is in it because it is the protected incremental object. A
...
#: The protected incremental object, as the catalogue keys it. Its load reads
