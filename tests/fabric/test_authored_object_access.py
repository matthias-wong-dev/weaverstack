...
emit({
    ...
    "resolved_table_path": resolver.delta_table(
        delta_target,
        "Sales",
        "Order",
    ).value,
...
def test_a_folder_resolves_to_the_directory_the_build_created(
    reached,
    lakehouse_estate,
):
    ...
    # The environment-side resolver may use a different transport representation
    # from the resolver running inside Spark, so use it only to verify existence.
    env = lakehouse_estate.env
    folder = env.resolver.folder_object(
        FolderTarget(lakehouse=env.target),
        "Sales",
        "OrderExport",
    )
